...
[`addCssToHead`](#addCssToHead) - Inject inline CSS into the <head> section of the figure's HTML.  
...
%% 2. Get a handle to the webwindow and the TextArea:
[hWin, widgetID] = mlapptools.getWebElements(hFig.TextArea);

%% 3. Define inline CSS and add it to the figure HTML:
cssText = ['"<style>'...
    '@-webkit-keyframes mymove {50% {background-color: blue;}}'...
    '@keyframes mymove {50% {background-color: blue;}}</style>"'];
mlapptools.addCssToHead(hWin, cssText);

%% 4. Add animation to control:      
